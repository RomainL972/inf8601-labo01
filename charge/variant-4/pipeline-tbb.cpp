...
// logiques de la machine plutot qu'une constante fixe. ×2 (comme l'ancienne
// version legacy de ce laboratoire, qui faisait déjà `cores * 2`) pour garder
// le pipeline saturé même quand une étage (ex.: l'écriture PNG) bloque le
// temps d'un appel système.
...
namespace {
// (image, valeur du canal bleu à ajouter). La valeur est calculée à l'étage de
...
// produisant exactement la même séquence de valeurs (2, 4, 6, ... mod 256)
...
int pipeline_tbb(image_dir_t* image_dir) {
    ...
    // `mutable` lambda capturing `next_blue` by value won't compile here --
    ...
    auto next_blue = std::make_shared<unsigned char>(0);

    tbb::parallel_pipeline(
        num_tokens(),
        tbb::make_filter<void, loaded_image_t>(
            tbb::filter_mode::serial_in_order,
            [image_dir, next_blue](tbb::flow_control& fc) -> loaded_image_t {
                image_t* image = image_dir_load_next(image_dir);
                if (image == NULL) {
                    fc.stop();
                    return {nullptr, 0};
                }
                *next_blue = (*next_blue + 2) % 256;
                return {image, *next_blue};
            }) &
            tbb::make_filter<loaded_image_t, loaded_image_t>(
                tbb::filter_mode::parallel,
                [](loaded_image_t in) -> loaded_image_t {
                    image_t* new_image = filter_scale_up(in.first, 2);
            ...
            // parallel, not serial_in_order: image_dir_save names its output file
            // from image->id (set once at load time and propagated by every
            // filter via image_create(image->id, ...), never from arrival
            // order), so concurrent saves are safe -- and, per the lab's legacy
            // TBB solution (~/git/inf8601/labo-1/source/pipeline-tbb.cpp,
            // SaveFilter), pinning the PNG encode+write to one thread was
            // exactly what capped throughput even with every other filter
            // already parallel.
