...
queue_t* queue_create(size_t size) {
    queue_t* queue = static_cast<queue_t*>(calloc(sizeof(*queue), 1));
    if (queue == NULL) {
...
void queue_destroy(queue_t* queue) {
    ...
    while (queue->head != NULL) {
        std::unique_ptr<queue_node_t> head(queue->head);
...
int queue_push(queue_t* queue, void* ptr) {
    std::unique_ptr<queue_node_t> node(new (std::nothrow) queue_node_t);
    if (!node) {
    ...
    /* the intrusive linked list now owns *node; queue_pop()/queue_destroy() delete it */
    node.release();
